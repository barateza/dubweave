...
def ingest_local_file(upload_path: str, job_dir: Path, logs: list) -> tuple[Path, Path, str, float, list]:
    ...
    if src.suffix.lower() == ".mp4" and codec_name in {"h264", "hevc"}:
        ...
        log(f"   Re-encoding {src.suffix} → mp4 (optimized for basic CPU)…", logs)
        subprocess.run(["ffmpeg", "-y", "-i", str(src), "-c:v", "libx264", "-preset", "ultrafast", "-crf", "26", "-c:a", "aac", "-b:a", "192k", "-movflags", "+faststart", str(video_path)], check=True)
