...
PROXY_CPS = 18.3 # 1.65 * ground truth (11.1)
...
def main():
    ...
    print(f"{'MinW':>6} | {'S':>8} | {'fit':>8} | {'sweet':>8} | {'n_segs':>8}")
    print("-" * 50)
    
    # Sweep min_words from 4 to 12
    for min_w in range(4, 13):
        output = run_benchmark(f"min_words_sweep: {min_w}", min_w)
        ...
        print(f"{min_w:6} | {s:8} | {fit:8} | {sweet:8} | {n:8}")
